...
if not USE_PHYSICAL:
    ...
    def _load_preload_cameras():
        """Load optional startup cameras from STRETCH_PRELOAD_CAMERAS."""
        raw = os.getenv("STRETCH_PRELOAD_CAMERAS", "").strip()
        if not raw:
            return []

        aliases = {
            "head_rgb": StretchCameras.cam_d435i_rgb,
            "head_depth": StretchCameras.cam_d435i_depth,
            "wrist_rgb": StretchCameras.cam_d405_rgb,
            "wrist_depth": StretchCameras.cam_d405_depth,
            "nav_rgb": StretchCameras.cam_nav_rgb,
        }
        cameras = []
        for token in raw.replace(";", ",").split(","):
            ...
            if not name:
                continue
            camera = aliases.get(name)
            if camera is None:
                try:
                    camera = StretchCameras[name]
                except KeyError:
                    print(f"[stretch_toolkit] Warning: Unknown preload camera {name!r}")
                    continue
    ...
    def _get_controller():
        ...
        if _controller is None:
            ...
            sim_kwargs = {'cameras_to_use': _load_preload_cameras()}

            camera_hz = os.getenv("STRETCH_CAMERA_HZ", "").strip()
            if camera_hz:
                try:
                    sim_kwargs["camera_hz"] = float(camera_hz)
                except ValueError:
                    print(f"[stretch_toolkit] Warning: Invalid STRETCH_CAMERA_HZ={camera_hz!r}")
