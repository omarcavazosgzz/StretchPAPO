...
def _watchdog_loop():
    ...
    while _watchdog_running:
        ...
        if os.getenv("STRETCH_KEEP_CAMERAS", "0") == "1":
            continue
        ...
        for camera in cameras_to_remove:
            try:
                ...
                _camera_startup_until.pop(camera, None)
            except:
                pass
...
_camera_startup_until = {}
CAMERA_STARTUP_GRACE_S = 3.0
...
def _get_sim_camera_frame(camera_enum):
    ...
    current_time = time.perf_counter()
    keep_cameras = os.getenv("STRETCH_KEEP_CAMERAS", "0") == "1"
    ...
    # Auto-register camera if not already active. The server needs a render
    # cycle or two before the first frame appears, so give new cameras a grace
    # period before treating None frames as failures.
    if camera_enum not in _sim.get_active_cameras():
        _sim.add_camera(camera_enum)
        _camera_startup_until[camera_enum] = current_time + CAMERA_STARTUP_GRACE_S
    ...
    try:
        ...
        if frame is None:
            startup_until = _camera_startup_until.get(camera_enum, 0.0)
            if keep_cameras or current_time < startup_until:
                return None
            ...
            _camera_startup_until.pop(camera_enum, None)
